...
func (s *Service) CreateRecord(ctx context.Context, projectID, tenantID, collectionID string, data map[string]any) (*Record, error) {
...
func (s *Service) ListRecords(ctx context.Context, projectID, tenantID, collectionID string, cursor string, limit int) ([]Record, string, error) {
	...
	query := `SELECT id, project_id, tenant_id, collection_id, data, created_at, updated_at
	           FROM records
	           WHERE project_id = $1 AND tenant_id = $2 AND collection_id = $3`
	args := []any{projectID, tenantID, collectionID}

	if cursor != "" {
		query += ` AND id > $4 ORDER BY created_at, id LIMIT $5`
...
func (s *Service) GetRecord(ctx context.Context, recordID, projectID, tenantID string) (*Record, error) {
	var r Record
	err := s.pool.QueryRow(ctx,
		`SELECT id, project_id, tenant_id, collection_id, data, created_at, updated_at
		 FROM records WHERE id = $1 AND project_id = $2 AND tenant_id = $3`,
		recordID, projectID, tenantID,
	).Scan(&r.ID, &r.ProjectID, &r.TenantID, &r.CollectionID, &r.Data, &r.CreatedAt, &r.UpdatedAt)
	if err != nil {
...
func (s *Service) UpdateRecord(ctx context.Context, recordID, projectID, tenantID string, data map[string]any) (*Record, error) {
	...
	err = s.pool.QueryRow(ctx,
		`UPDATE records SET data = $1, updated_at = now()
		 ...
		 RETURNING id, project_id, tenant_id, collection_id, data, created_at, updated_at`,
		data, recordID, projectID, tenantID,
	).Scan(&r.ID, &r.ProjectID, &r.TenantID, &r.CollectionID, &r.Data, &r.CreatedAt, &r.UpdatedAt)
	if err != nil {
...
func (s *Service) DeleteRecord(ctx context.Context, recordID, projectID, tenantID string) error {
	_, err := s.pool.Exec(ctx,
		`DELETE FROM records WHERE id = $1 AND project_id = $2 AND tenant_id = $3`,
		recordID, projectID, tenantID,
	)
	if err != nil {
